...
// 导入simple_html_dom包
	...
	require_once('database.php');  //加载数据库处理函数　
...
//解析每个url内容
	...
	for($i = 0; $i<$count; $i++){
		...
		$resultBeginContent = $contentHtml->find('articlepagebegin p');  /*　获得id为content_title  的标题　*/
		$resultContent = "";
		foreach($resultBeginContent as $element){
			$resultContent=$resultContent.$element->innertext(); 
		}
		...
		//// 字符转码 成utf8 
	//	$resultContent = iconv("GBK", "UTF-8", $resultContent);
