...
$xml=simplexml_load_file("wu_ba_tuan_gou_xml.xml");  //加载xml文件
$time=time();        //获得今天时间戳
$from="wu_ba_tuan_gou";        //接口数据的来源
$arr=array();       //数据数组
echo "begin,please wait...";
foreach($xml->url as $data){
	//$deal_more_img = $data->deal->deal_more_img;
	//echo $data->deal->deal_more_img;
	//if(strstr($data->data->shops->shop->area,"南坪")||strstr($data->data->shops->shop->area,"南岸")){
		$arr[] = array(
			'deal_catch_id' => $data->data->display->id,
			'deal_more_img' =>'',
			'deal_title' => $data->data->display->title,
			'deal_img' => $data->data->display->image,
			'deal_cate' => $data->data->display->category,
